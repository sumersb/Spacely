...
export default function BathroomCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setBathroomNum(initialValues.bathroomNum);
    setBathroomDetails(initialValues.bathroomDetails);
    setErrors({});
  };
  const validations = {
    bathroomNum: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new Bathroom(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "BathroomCreateForm")}
    >
      <TextField
        label="Bathroom num"
        isRequired={false}
