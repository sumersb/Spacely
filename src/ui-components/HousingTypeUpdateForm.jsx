...
export default function HousingTypeUpdateForm(props) {
  const {
    id,
    housingType,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...housingTypeRecord };
    setHouse(cleanValues.house);
    setApartment(cleanValues.apartment);
    setCondo(cleanValues.condo);
    setTownhouse(cleanValues.townhouse);
    setHousingDetail(cleanValues.housingDetail);
    setErrors({});
  };
  const [housingTypeRecord, setHousingTypeRecord] = React.useState(housingType);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(HousingType, id) : housingType;
      setHousingTypeRecord(record);
    };
    queryData();
  }, [id, housingType]);
  React.useEffect(resetStateValues, [housingTypeRecord]);
  const validations = {
    house: [],
    apartment: [],
    condo: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            HousingType.copyOf(housingTypeRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "HousingTypeUpdateForm")}
    >
      <SwitchField
        label="House"
