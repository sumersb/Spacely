...
export default function FurnishingCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setFurnish(initialValues.furnish);
    setFurnishDetail(initialValues.furnishDetail);
    setErrors({});
  };
  const validations = {
    furnish: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new Furnishing(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "FurnishingCreateForm")}
    >
      <SwitchField
        label="Furnish"
