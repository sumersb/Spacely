...
export default function LeasingPeriodCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setStarting(initialValues.starting);
    setEnding(initialValues.ending);
    setExtention(initialValues.extention);
    setAvailabilityDetails(initialValues.availabilityDetails);
    setShortTerm(initialValues.shortTerm);
    setLongTerm(initialValues.longTerm);
    setErrors({});
  };
  const validations = {
    starting: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(new LeasingPeriod(modelFields));
          if (onSuccess) {
      ...
      {...rest}
      {...getOverrideProps(overrides, "LeasingPeriodCreateForm")}
    >
      <TextField
        label="Starting"
        isRequired={false}
