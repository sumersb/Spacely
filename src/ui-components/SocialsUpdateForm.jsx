...
export default function SocialsUpdateForm(props) {
  const {
    id,
    socials,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...socialsRecord };
    setInstaLink(cleanValues.instaLink);
    setFacebookLink(cleanValues.facebookLink);
    setOtherLink(cleanValues.otherLink);
    setInstahandler(cleanValues.instahandler);
    setSocialDetails(cleanValues.socialDetails);
    setErrors({});
  };
  const [socialsRecord, setSocialsRecord] = React.useState(socials);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Socials, id) : socials;
      setSocialsRecord(record);
    };
    queryData();
  }, [id, socials]);
  React.useEffect(resetStateValues, [socialsRecord]);
  const validations = {
    instaLink: [{ type: "URL" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Socials.copyOf(socialsRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "SocialsUpdateForm")}
    >
      <TextField
        label="Insta link"
        isRequired={false}
